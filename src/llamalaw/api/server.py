...
from llamalaw.models.contract import Contract, Clause, Risk, Party
...
class AnalyzeRequest(BaseModel):
    ...
    save_to_repository: bool = Field(False, description="Whether to save results to repository")
...
@app.post("/analyze", response_model=Union[AnalysisResult, ErrorResponse], status_code=200)
...
def get_contract_text(
    ...
    try:
        ...
        if text is None:
            raise HTTPException(status_code=404, detail=f"Document text not found for contract '{contract_id}'")
        ...
        raise HTTPException(status_code=500, detail=f"Error getting contract text: {str(e)}")
...
def get_contract_risks(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error getting contract risks: {str(e)}")
...
def get_contract_clauses(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error getting contract clauses: {str(e)}")
...
def delete_contract(
    ...
    try:
        ...
        if not success:
            raise HTTPException(status_code=500, detail=f"Failed to delete contract '{contract_id}'")
