...
def setup_parser() -> argparse.ArgumentParser:
    ...
    analyze_parser.add_argument(
        "document", 
        help="Path to document file or directory to analyze"
    )
    analyze_parser.add_argument(
        "--output", "-o",
        help="Output format (json, text, summary)",
        default="summary"
    )
    analyze_parser.add_argument(
        "--save", "-s",
        help="Save analysis results to repository",
        action="store_true"
    )
    
    # List command
    list_parser = subparsers.add_parser(
        "list", help="List contracts in repository"
    )
    ...
    delete_parser = subparsers.add_parser(
        "delete", help="Delete a contract by ID"
    )
    ...
    stats_parser = subparsers.add_parser(
        "stats", help="Show repository statistics"
    )
...
def delete_contract(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        # Confirm deletion
        if not force:
            confirm = input(f"Are you sure you want to delete contract '{contract.title}'? (y/n): ")
            if confirm.lower() != 'y':
...
def print_table(headers: List[str], rows: List[List[Any]]) -> None:
    ...
    widths = [max(len(str(h)), max([len(row[i]) for row in string_rows], default=0))
