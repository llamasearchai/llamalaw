...
from llamalaw.models.contract import Contract, Party, Clause, Risk, ContractDates
...
class ContractRecord(Base):
    ...
    def to_contract(self) -> Contract:
        ...
        dates = ContractDates(
            ...
            termination=self.termination_date
        )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self.Session() as session:
            ...
            for (doc_type,) in types:
                count = session.query(ContractRecord).filter_by(document_type=doc_type).count()
                type_counts[doc_type] = count
                
            # Get average risk score
            risk_query = session.query(ContractRecord.risk_score).filter(ContractRecord.risk_score.isnot(None))
            risk_scores = [row[0] for row in risk_query.all()]
            avg_risk = sum(risk_scores) / len(risk_scores) if risk_scores else 0
            
            # Get date ranges
            date_query = session.query(
                ContractRecord.created_at,
