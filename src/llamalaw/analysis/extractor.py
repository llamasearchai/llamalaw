...
from llamalaw.models.contract import Clause, Party, ContractDates
...
class EntityExtractor:
    ...
    def _compile_patterns(self) -> None:
        ...
        self.date_patterns = [
            ...
            re.compile(r'\b(0?[1-9]|1[0-2])[-/](0?[1-9]|[12][0-9]|3[01])[-/](19|20)\d\d\b'),
            # Month DD, YYYY
            re.compile(r'\b(January|February|March|April|May|June|July|August|September|October|November|December)\s+(0?[1-9]|[12][0-9]|3[01])(st|nd|rd|th)?,\s+(19|20)\d\d\b'),
            # Abbreviated month
            re.compile(r'\b(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\.?\s+(0?[1-9]|[12][0-9]|3[01])(st|nd|rd|th)?,\s+(19|20)\d\d\b'),
            # YYYY-MM-DD (ISO format)
            re.compile(r'\b(19|20)\d\d[-](0?[1-9]|1[0-2])[-](0?[1-9]|[12][0-9]|3[01])\b'),
        ]
        ...
        self.money_patterns = [
            ...
            re.compile(r'£\s*(\d{1,3}(?:,\d{3})*(?:\.\d{1,2})?)\b'),
            # Written out
            re.compile(r'\b(\d{1,3}(?:,\d{3})*(?:\.\d{1,2})?\s*(?:dollars|euros|pounds))\b'),
            # USD, EUR, etc.
        ...
        self.org_patterns = [
            re.compile(r'\b([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*(?:,)?\s+(?:Inc|LLC|Ltd|Corporation|Corp|Company|Co|GmbH|LP|LLP|Limited|International))\b'),
            re.compile(r'\b((?:The\s+)?[A-Z][a-z]+(?:\s+[A-Z][a-z]+)*\s+(?:Trust|Foundation|Association|Institute|University|College|School))\b'),
        ]
...
class ClauseExtractor:
    ...
    def __init__(self):
        ...
        self.clause_types = {
            "definition": [
                "definitions", "defined terms", "interpretation", "meaning of"
            ],
            "payment": [
                "payment", "consideration", "fees", "compensation", "price"
            ],
            "term": [
                "term of", "duration", "period", "agreement term"
            ],
            "termination": [
            ...
            "limitation-of-liability": [
                "limitation of liability", "limited liability", "liability cap", "limitation on liability"
            ],
            "assignment": [
                "assignment", "assignability", "transfer of"
            ],
            "force-majeure": [
                "force majeure", "act of god", "events beyond control"
            ],
            "notice": [
                "notice", "notification", "communications"
            ],
            "amendment": [
                "amendment", "modification", "changes to this agreement"
            ],
            "dispute-resolution": [
    ...
    def _determine_clause_type(self, title: str, content: str) -> str:
        ...
        for clause_type, indicators in self.clause_types.items():
            for indicator in indicators:
                if indicator.lower() in content_lower[:100]:  # Check only first 100 chars
                    return clause_type
