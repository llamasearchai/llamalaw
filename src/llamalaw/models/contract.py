...
class Risk(BaseModel):
    ...
    severity: RiskSeverity = Field(default=RiskSeverity.MEDIUM, description="Severity level")
    clause_id: Optional[str] = Field(None, description="ID of the clause containing the risk")
    section: Optional[str] = Field(None, description="Section containing the risk")
    recommendations: List[str] = Field(default_factory=list, description="Recommendations for mitigation")
...
class Party(BaseModel):
    ...
    type: Optional[str] = Field(None, description="Type of party (individual, company, etc.)")
...
class Obligation(BaseModel):
    ...
    clause_id: str = Field(..., description="ID of the clause containing the obligation")
    responsible_party: Optional[str] = Field(None, description="Party responsible for the obligation")
    beneficiary_party: Optional[str] = Field(None, description="Party benefiting from the obligation")
    deadline: Optional[Union[date, str]] = Field(None, description="Deadline for the obligation")
...
class ContractDates(BaseModel):
    ...
    termination: Optional[Union[date, str]] = Field(None, description="Termination date")


class Term(BaseModel):
    """Term information for a contract"""
    length: Optional[str] = Field(None, description="Length of the term")
    start_date: Optional[Union[date, str]] = Field(None, description="Start date of the term")
    end_date: Optional[Union[date, str]] = Field(None, description="End date of the term")
    ...
    termination_notice: Optional[str] = Field(None, description="Notice required for termination")
...
class Contract(BaseModel):
    ...
    parties: List[Party] = Field(default_factory=list, description="Parties to the contract")
    dates: ContractDates = Field(default_factory=ContractDates, description="Important dates")
    term: Optional[Term] = Field(None, description="Term information")
    clauses: List[Clause] = Field(default_factory=list, description="Clauses in the contract")
    obligations: List[Obligation] = Field(default_factory=list, description="Obligations in the contract")
