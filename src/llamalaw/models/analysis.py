...
from llamalaw.models.contract import Clause, Contract, Risk
...
class RiskAssessmentResult(BaseModel):
    ...
    risk_categories: Dict[str, int] = Field(
        default_factory=dict, description="Risk counts by category"
    )
    risk_severity: Dict[str, int] = Field(
        default_factory=dict, description="Risk counts by severity"
    )
    metadata: Dict[str, Any] = Field(
        default_factory=dict, description="Additional metadata"
    )
...
class AnalysisResult(BaseModel):
    ...
    def generate_summary(self) -> str:
        ...
        risks_str = ", ".join(
            f"{count} {sev}" for sev, count in risk_severity_counts.items()
        )
